<template>
	<view class="uni-goods-nav">
		...
		<view class="uni-tab__cart-box flex">
			<view v-if="options && options.length" class="flex uni-tab__cart-sub-left">
			...
			<view :class="{'uni-tab__right':fill}" class="flex uni-tab__cart-sub-right ">
				<view v-for="(item,index) in buttonGroup" :key="index" :style="{backgroundColor:item.backgroundColor,color:item.color}"
				 class="flex uni-tab__cart-button-right" @click="buttonClick(index,item)">
					...
					<text class="uni-tab__cart-button-right-text">
						<uni-icons class="uni-tab__group_icon" v-if="item.icon" :type="item.icon" size="20" color="#646566"></uni-icons>{{ item.text }}
					</text></view>
